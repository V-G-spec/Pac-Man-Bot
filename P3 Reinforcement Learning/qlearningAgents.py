...
#             maxi = max(maxi, self.qvalue(state, action))
...
#             return None
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.qvalue[(state, action)] = (self.qvalue[(state, action)]*(1-lr)) + (lr*sample)
...
#         print(features)
        sumi=0.0
#         for wt, ft in zip(weight, features):
#             sumi+=(wt*ft)
        for ft in features:
            sumi+=(weight[ft]*features[ft])
        return sumi
        "*** YOUR CODE HERE ***"
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        newVal = reward+ (self.discount*maxNewQ)
        diff = newVal-currVal
        for ft in features:
            weight[ft] += (self.alpha*diff*features[ft])
#         return 
        "*** YOUR CODE HERE ***"
#         util.raiseNotDefined()
